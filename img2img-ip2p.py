...
def main(args):
    ...
    out_filepath = os.path.join(dir_path, f"reimagined-{start_time}.png")
    ...
    if isinstance(req_img, list):
        imgs_list = req_img
        req_img = imgs_list[0]
        req_img2 = imgs_list[len(imgs_list)-1]

    if test_mode:
        req_img = os.path.join(APP_PATH, "sample.png")
        req_img2 = os.path.join(APP_PATH, "sample.png")
    #print(req_img)
    #exit(1)
    ...
    # Read Image in RGB order
    img2 = cv2.imread(req_img2)

    # Encode into PNG and send to ControlNet
    retval2, bytes2 = cv2.imencode('.png', img2)
    encoded_image2 = base64.b64encode(bytes2).decode('utf-8')
