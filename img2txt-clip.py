...
def main(args):
    ...
    if req_text and encoded_image:
        ...
        response = requests.post(url=f'{url}/sdapi/v1/interrogate', json=payload, timeout=1280)

        r = response.json()
        out_filepath = os.path.join(dir_path, f'output_{args.model_id}.txt')
        with open(out_filepath, 'w', encoding='utf-8') as output_file:
            output_file.write(str(r['caption']))
